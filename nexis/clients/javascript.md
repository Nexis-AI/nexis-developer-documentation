...
title: JavaScript Client for Nexis
sidebarSortOrder: 2
---

## What is Nexis-Web3.js?

The Nexis-Web3.js library aims to provide complete coverage of Nexis. The
library was built on top of the [Nexis JSON RPC API](/docs/rpc).
...
[Nexis terminology](/docs/terminology.md#cross-program-invocation-cpi)
...
```bash
yarn add @Nexisnetwork/web3.js
```

#### npm

```bash
npm install --save @Nexisnetwork/web3.js
```
...
<script src="https://unpkg.com/@Nexisnetwork/web3.js@latest/lib/index.iife.js"></script>

<!-- Production (minified) -->
<script src="https://unpkg.com/@Nexisnetwork/web3.js@latest/lib/index.iife.min.js"></script>
```
...
```javascript
const NexisWeb3 = require("@Nexisnetwork/web3.js");
...
```javascript
import * as NexisWeb3 from "@Nexisnetwork/web3.js";
console.log(NexisWeb3);
```
...
```javascript
// NexisWeb3 is provided in the global namespace by the bundle script
console.log(NexisWeb3);
```
...
To allow users to use your dApp or application on Nexis, they will need to get
...
[wallet-adapter](https://github.com/Nexis-labs/wallet-adapter) library.

### Creating and Sending Transactions

To interact with programs on Nexis, you create, sign, and send transactions to
...
A transaction in Nexis-Web3.js is created using the
...
order to define which Nexis network you are connecting to, namely
`mainnet-beta`, `testnet`, or `devnet`.

### Interacting with Custom Programs

The previous section visits sending basic transactions. In Nexis everything you
do interacts with different programs, including the previous section's transfer
transaction. At the time of writing programs on Nexis are either written in
Rust or C.

Let's look at the `SystemProgram`. The method signature for allocating space in
your account on Nexis in Rust looks like this:
...
In Nexis when you want to interact with a program you must first know all the
...
Let's look at how to call this instruction using Nexis-web3.js:
...
nzt so we can pay for the allocate transaction.
...
The above is created using `u32` and `ns64` from `@Nexisnetwork/buffer-layout` to
...
encoding it correctly for Rust programs on Nexis to interpret.
...
```rust
/* https://github.com/Nexis-labs/Nexis/blob/21bc43ed58c63c827ba4db30426965ef3e807180/sdk/program/src/system_instruction.rs#L142-L305 */
...
integers. `ns64` comes from `@Nexisnetwork/buffer-layout` to help with type
conversions between Rust and Javascript. You can find more type conversions
between Rust and Javascript at
[Nexis-labs/buffer-layout](https://github.com/Nexis-labs/buffer-layout).
...
```javascript
const { struct, u32, ns64 } = require("@Nexisnetwork/buffer-layout");
