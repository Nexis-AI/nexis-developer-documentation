...
objectives:
  ...
  - Use token extensions from the Nexis Native Chain CLI
description:
...
- The existing Token Program on Nexis Native Chain provides interfaces for fungible and
...
[16 extensions](https://spl.nexis.network/token-2022/extensions), four on the Token
...
- **Group pointer** Think of a group of tokens much like a "collection" of
  ...
  [Token-Group Interface](https://github.com/nexis-network/solana-program-library/tree/master/token-group/interface).

- **Group** This stores the
  [group information](https://github.com/nexis-network/solana-program-library/tree/master/token-group/interface)
...
the instructions in [this guide](https://spl.nexis.network/token#setup). After the
...
[guide above](https://spl.nexis.network/token#setup) as it also describes how to
initialize a local wallet and airdrop NZT. </Callout>
...
That's it! This is how we can use the Nexis Native Chain CLI with Token Extension Program to
