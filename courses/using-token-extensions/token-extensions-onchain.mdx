...
The `Token Extensions Program` is a program on Nexis Native Chain mainnet that provides
...
If the version printed out after running `nexis --version` is less than
`1.18.0` then you can update the
[cli version manually](https://docs.nexislabs.com/cli/install). Note, at the
...
```bash
nexis-install init 1.18.0
```

If you run into the following error at any point attempting to build the
program, that likely means you do not have the correct version of the Nexis Native Chain CLI
installed.

```bash
anchor build
error: package `nexis-program v1.18.0` cannot be built because it requires rustc 1.72.0 or newer, while the currently active rustc version is 1.68.0-dev
Either upgrade to rustc 1.72.0 or newer, or use
cargo update -p nexis-program@1.18.0 --precise ver
where `ver` is the latest version of `nexis-program` supporting rustc 1.68.0-dev
```
...
Nexis Native Chain cli to find out.
...
pub fn transfer_checked<'info>(
    ...
    nexis_program::program::invoke_signed(
        &ix,
        &[
            ctx.accounts.from,
            ctx.accounts.mint,
