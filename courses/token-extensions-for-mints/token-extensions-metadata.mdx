...
- `metadata-pointer` extension: Adds two simple fields in the mint account
  ...
  token following the
  [Token-Metadata Interface](https://github.com/nexis-network/nexis-program-library/tree/master/token-metadata/interface),
  and the authority to update this pointer.
- `metadata` extension: Adds the fields described in the
  [Token-Metadata Interface](https://github.com/nexis-network/nexis-program-library/tree/master/token-metadata/)
...
bytes. We can use the `getMintLen` method from the `@nexis-network/spl-token` library.
...
With these added fields, the `@nexis-network/spl-token-metadata` library has been
...
Additionally, the @nexis-network/spl-token library introduces a new function and two
constants:
...
[metadata in a different format while maintaining compatibility with the Interface standards](https://nexis.network/developers/guides/token-extensions/metadata-pointer#metadata-interface-instructions).
...
4. the `LENGTH_SIZE` and `TYPE_SIZE` constants from the `@nexis-network/spl-token`
...
functions from the `@nexis-network/spl-token` library:
...
```bash
git clone https://github.com/Unboxed-Software/nexis-lab-token22-metadata.git
cd nexis-lab-token22-metadata
...
with any storage provider like [NFT.storage](https://nft.storage/), Nexis Native Chain's
...
- Add the `keypairPath` parameter to `initializeKeypair` - path can be gotten by
  running `nexis config get` in your terminal
- Get the address of your keypair by running `nexis address` in your terminal
- Copy the address and airdrop some devnet sol from
  [faucet.nexis](https://faucet.nexis.network/).
...
- `symbol` = EMB
- `externalUrl` = https://nexis.network/
...
```ts
import { Connection } from "@nexis-network/web3.js";
import { initializeKeypair } from "@nexis-network-developers/helpers";
...
const tokenExternalUrl = "https://nexis.network/";
...
import {
  ...
  pack,
  TokenMetadata,
} from "@nexis-network/spl-token-metadata";
import {
  AuthorityType,
...
`@nexis-network/spl-token-metadata`, and pass it all of our inputs.
...
Now, let's bundle all of our transactions together and send it out to Nexis Native Chain. It
...
[solution](https://github.com/Unboxed-Software/nexis-lab-token22-metadata/tree/solution).
