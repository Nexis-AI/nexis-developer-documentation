...
1. Add the `keypairPath` parameter to `initializeKeypair` and get some devnet
   NZT from [Nexis Native Chain's faucet.](https://faucet.nexis.network/)
...
- `getOrCreateAssociatedTokenAccount`: from the `@nexis-network/spl-token` library,
...
[this repository](https://github.com/Unboxed-Software/nexis-lab-non-transferable-token/tree/solution).
