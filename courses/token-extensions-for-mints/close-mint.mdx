...
Finally we add the instructions to the transaction and submit it to the Nexis Native Chain
network.
...
```ts
import { Connection, Keypair, LAMPORTS_PER_SOL } from "@nexis-network/web3.js";
 ...
 * If a keypair exists, airdrop a NZT if needed.
 */
...
[faucet.nexis.network](https://faucet.nexis.network/) and airdrop some NZT to your
...
- `SystemProgram.createAccount`: Creates the instruction to allocates space on
  Nexis Native Chain for the mint account
...
[this repository](https://github.com/Unboxed-Software/nexis-lab-close-mint-account/tree/solution).
