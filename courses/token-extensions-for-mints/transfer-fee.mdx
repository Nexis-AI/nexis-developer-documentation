...
[creation with the JS library](https://nexis.stackexchange.com/questions/7775/spl-token-2022-how-to-modify-transfer-fee-configuration-for-an-existing-mint).
However you can from the CLI assuming the result of `nexis config` wallet is
...
import { transferCheckedWithFee } from "@nexis-network/spl-token";

/**
 * Create a connection and initialize a keypair if one doesn't already exists.
 * If a keypair exists, airdrop a NZT token if needed.
 */
...
In a separate terminal, run the following command: `nexis-test-validator`. This
...
`clusterApiUrl` from `@nexis-network/web3.js` and pass it to the connection as such:
...
running `nexis config get` in your terminal. And then go to
[faucet.nexis.network](https://faucet.nexis.network/) and airdrop some sol to your
address. You can get your address from running `nexis address` in your
terminal.
...
[this repository](https://github.com/Unboxed-Software/nexis-lab-transfer-fee/tree/solution).
