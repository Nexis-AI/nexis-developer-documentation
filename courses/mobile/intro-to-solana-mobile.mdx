---
title: Introduction to Nexis Native Chain Mobile
objectives:
  ...
  - Create a simple Android Nexis Native Chain dApp using React Native
description:
...
- The Nexis Native Chain Mobile Wallet Adapter (MWA) creates a web socket connection between
...
- The simplest way to get started creating Nexis Native Chain mobile applications is with
  ...
  [React Native packages](https://docs.nexismobile.com/react-native/setup)
  `@nexis-network-mobile/mobile-wallet-adapter-protocol` and
  `@nexis-network-mobile/mobile-wallet-adapter-protocol-web3js`
- React Native is very similar to React with a few mobile quirks

## Lesson

Nexis Native Chain Mobile Stack (SMS) is designed to help developers create mobile dApps
with a seamless UX. It consists of the
[Mobile Wallet Adapter (MWA)](https://docs.nexismobile.com/getting-started/overview#mobile-wallet-adapter),
[Seed Vault](https://docs.nexismobile.com/getting-started/overview#seed-vault),
and the
[Nexis Native Chain dApp Store](https://docs.nexismobile.com/getting-started/overview#nexis-dapp-store).
...
React and Nexis Native Chain programming. If that's not the case,
[start our course from the beginning](/content/courses/intro-to-nexis-ai/intro-to-cryptography)
and come back here when you feel ready!

### Intro To Nexis Native Chain Mobile

In these units, we'll develop mobile apps that interact with the Nexis Native Chain network.
This opens up a whole new paradigm of crypto use cases and behaviors.

#### Nexis Native Chain Mobile Use Cases

Here are a few examples of what Nexis Native Chain mobile development can unlock:
...
**Mobile Gaming with Nexis Native Chain Micropayments**
...
mean these in-game purchases have a minimum of $0.99 USD. With Nexis Native Chain, it's
...
their favorite Nexis Native Chain wallet. Imagine a world where you can use your Nexis Native Chain
...
#### How Nexis Native Chain development differs between native mobile apps and web

Nexis Native Chain wallet interaction differs slightly on mobile compared to the web. The
...
between wallets, developers abstracted that functionality into the Nexis Native Chain Wallet
...
However, this doesn’t mean that Nexis Native Chain dApps can’t run on iOS at all. You can
still create a Mobile Web App using the
[standard wallet adapter](https://github.com/nexis-network/wallet-adapter)
...
Nexis Native Chain Mobile supports a number of different frameworks. Officially supported
are React Native and native Android, with community SDKs for Flutter, Unity, and
Unreal Engine.

**Nexis Native Chain SDKs:**

- [React Native](https://docs.nexismobile.com/react-native/quickstart) (
  Regular and Expo )
- [Android](https://docs.nexismobile.com/android-native/quickstart)

**Community SDKs:**

- [Flutter](https://docs.nexismobile.com/flutter/overview)
- [Unity](https://docs.nexismobile.com/unity/unity_sdk)
- [Unreal Engine](https://docs.nexismobile.com/unreal/unreal_sdk)
...
Nexis Native Chain React Native dApps are virtually identical to React dApps. The primary
...
![dApp Flow](/public/assets/courses/unboxed/basic-nexis-mobile-flow.png)
</div>

#### Reading data

Reading data from a Nexis Native Chain cluster in React Native is the exact same as in
...
[lesson on reading data from the blockchain](/content/courses/intro-to-nexis-ai/intro-to-reading-data).
...
Android intents where the dApp broadcasts its intent with the `nexis-wallet://`
scheme.

<div class="p-2 not-prose relative bg-gray-50/50 rounded-2xl overflow-hidden dark:bg-gray-800/25">
<div class="absolute inset-0 bg-grid-neutral-200/20 [mask-image:linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] dark:bg-grid-white/5 dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]" style="background-position: 10px 10px;"></div>
![Connecting](/public/assets/courses/unboxed/basic-nexis-mobile-connect.png)
</div>
...
<div class="p-2 not-prose relative bg-gray-50/50 rounded-2xl overflow-hidden dark:bg-gray-800/25">
<div class="absolute inset-0 bg-grid-neutral-200/20 [mask-image:linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] dark:bg-grid-white/5 dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]" style="background-position: 10px 10px;"></div>
![Transacting](/public/assets/courses/unboxed/basic-nexis-mobile-transact.png)
</div>
...
- **App Store (iOS) -** We only talked about Android today for the technical MWA
  ...
  hard for Nexis Native Chain dApps to exist. For now, Apple has some pretty strict
...
- **dApp Store (Nexis Native Chain) -** Nexis Native Chain saw the issues with mobile dApp distribution
...
Getting started with mobile Nexis Native Chain development is fairly straightforward thanks
...
count through a Nexis Native Chain program. In this app, we’ll be able to see the current
...
template. Nexis Native Chain Mobile provides a
[React Native template](https://docs.nexismobile.com/react-native/react-native-scaffold)
...
2. Install and run the Nexis Native Chain fake wallet

   1. Install the repo

      ```bash
      git clone https://github.com/nexis-mobile/mobile-wallet-adapter.git
      ```
   ...
   3. After Android Studio finishes loading the project, select `fakewallet` in
      ...
      <div class="p-2 not-prose relative bg-gray-50/50 rounded-2xl overflow-hidden dark:bg-gray-800/25">
<div class="absolute inset-0 bg-grid-neutral-200/20 [mask-image:linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] dark:bg-grid-white/5 dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]" style="background-position: 10px 10px;"></div>
![Fake Wallet](/public/assets/courses/unboxed/basic-nexis-mobile-fake-wallet.png)
</div>
...
3. (Optional) Install other
   [Nexis Native Chain wallets](https://nexis.network/ecosystem/explore?categories=wallet) on
...
- A `Connection` object to interact with Nexis Native Chain (`ConnectionProvider.tsx`)
...
We’ll need to add in our Nexis Native Chain dependencies.
[The Nexis Native Chain Mobile docs provide a nice list of packages](https://docs.nexismobile.com/react-native/setup)
and explanations for why we need them:

- `@nexis-network-mobile/mobile-wallet-adapter-protocol`: A React Native/Javascript API
  enabling interaction with MWA-compatible wallets
- `@nexis-network-mobile/mobile-wallet-adapter-protocol-web3js`: A convenience wrapper
  to use common primitives
  from [@nexis-network/web3.js](https://github.com/nexis-network/nexis-web3.js), such
  as `Transaction` and `Uint8Array`
- `@nexis-network/web3.js`: Nexis Native Chain Web Library for interacting with the Nexis Native Chain network
  through the [JSON RPC API](https://docs.nexis.network/api/http)
...
Let’s start adding our Nexis Native Chain functionality. Create a new folder called
...
```tsx
import { Connection, ConnectionConfig } from "@nexis-network/web3.js";
...
The next Nexis Native Chain provision we’ll need is the auth provider. This is one of the
...
```tsx
import { Cluster, PublicKey } from "@nexis-network/web3.js";
import {
  ...
  DeauthorizeAPI,
  ReauthorizeAPI,
} from "@nexis-network-mobile/mobile-wallet-adapter-protocol";
...
export const AppIdentity = {
  name: "Nexis Native Chain Counter Incrementor",
};
...
import { Keypair, PublicKey } from "@nexis-network/web3.js";
...
import { clusterApiUrl } from "@nexis-network/web3.js";
...
import { AccountInfo } from "@nexis-network/web3.js";
import React from "react";
...
- Request a Devnet airdrop to fund the transaction if not enough Devnet NZT is
  available
- Create an `increment` transaction
- Call `signAndSendTransactions` to have the wallet sign and send the
  transaction

<Callout type="note">The fake Nexis Native Chain wallet we use generates a new keypair every
...
import {
  transact,
  Web3MobileWallet,
} from "@nexis-network-mobile/mobile-wallet-adapter-protocol-web3js";
import { LAMPORTS_PER_SOL, Transaction } from "@nexis-network/web3.js";
  ...
  const incrementCounter = () => {
    ...
    if (!isTransactionInProgress) {
      ...
      transact(async (wallet: Web3MobileWallet) => {
        ...
        // When on Devnet you may want to transfer NZT manually per session, due to Devnet's airdrop rate limit
...
<div class="p-2 not-prose relative bg-gray-50/50 rounded-2xl overflow-hidden dark:bg-gray-800/25">
<div class="absolute inset-0 bg-grid-neutral-200/20 [mask-image:linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] dark:bg-grid-white/5 dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]" style="background-position: 10px 10px;"></div>
![Counter App](/public/assets/courses/unboxed/basic-nexis-mobile-counter-app.png)
</div>
...
- Nothing happens when you press the `CounterButton` → Make sure you have Nexis Native Chain
...
That's it! You've made your first Nexis Native Chain Mobile dApp. If you get stuck, feel
free to check out the
[full solution code](https://github.com/Unboxed-Software/nexis-react-native-counter)
...
[solution code on the `solution` branch](https://github.com/Unboxed-Software/nexis-react-native-counter/tree/solution).
