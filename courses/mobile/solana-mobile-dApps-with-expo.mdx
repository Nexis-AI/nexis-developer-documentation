---
title: Building Nexis Native Chain Mobile dApps with Expo
objectives:
  - Create Nexis Native Chain dApps with Expo
  - Use mobile-specific peripherals and capabilities
  - Integrate ecosystem libraries into your mobile dApps
description: "How to use Nexis Native Chain in your Expo apps."
...
- A lot of Nexis Native Chain ecosystem libraries don't support React native out of the box,
  but you can typically use them with the right
  [polyfills](https://developer.mozilla.org/en-US/docs/Glossary/Polyfill)

## Lesson

So far in exploring Nexis Native Chain Mobile, we've used vanilla React Native to build very
...
2. How to integrate JS/TS libraries from the Nexis Native Chain ecosystem that don't
...
with the Nexis Native Chain mobile SDK. Coming from the
[Nexis Native Chain Expo setup article](https://docs.nexismobile.com/react-native/expo):
...
> Nexis Native Chain Mobile SDKs need. Instead, we'll need to use a custom development build
> which makes Nexis Native Chain Mobile React Native libraries (i.e Mobile Wallet Adapter)
...
[Introduction to Nexis Native Chain Mobile lesson](/content/courses/mobile/intro-to-nexis-ai-mobile).
...
Unfortunately, that won't work with Nexis Native Chain Mobile applications. Instead, you'll
...
When working with Nexis Native Chain specifically, the vast majority of ecosystem libraries
...
the standard Nexis Native Chain libraries but also for Metaplex.

#### Nexis Native Chain Polyfills

For a Nexis Native Chain + Expo app, you'll need the following:
...
Expo, Nexis Native Chain, and Metaplex all work together. We'll do this modularly so you'll
...
[Introduction to Nexis Native Chain Mobile lesson](/content/courses/mobile//basic-nexis-mobile.md#0-prerequisites)
...
```bash
npx create-expo-app -t expo-template-blank-typescript nexis-expo
cd nexis-expo
```
...
Nexis Native Chain Mobile to work correctly. We'll need to add an new config file that lets
...
### 2. Configure your Expo app to work with Nexis Native Chain

Now that we have an Expo app up and running, we need to add our Nexis Native Chain
...
#### 0. Install a Devnet-enabled Nexis Native Chain wallet
...
```bash
cd ..
git clone https://github.com/Unboxed-Software/react-native-fake-nexis-wallet
cd react-native-fake-nexis-wallet
git checkout solution
npm run install
```

The wallet should be installed on your emulator or device. Make sure to open the
newly installed wallet and airdrop yourself some NZT.
...
#### 1. Install Nexis Native Chain dependencies

We are going to install some basic Nexis Native Chain dependencies that are likely to be
needed by all Nexis Native Chain mobile apps. This will include some polyfills that allow
...
#### 3. Add Nexis Native Chain boilerplate providers

Next, let's add some Nexis Native Chain boilerplate that can springboard you into most
Nexis Native Chain-based apps.

Create two new folders: `components` and `screens`.

We are going to use some boilerplate code from the
[first Mobile lesson](/content/courses/mobile/basic-nexis-mobile). We will be
...
[of our existing Auth Provider from Github](https://raw.githubusercontent.com/Unboxed-Software/nexis-advance-mobile/main/components/AuthProvider.tsx)
into the new file.

Secondly, create file `components/ConnectionProvider.tsx` and copy the contents
[of our existing Connection Provider from Github](https://raw.githubusercontent.com/Unboxed-Software/nexis-advance-mobile/main/components/ConnectionProvider.tsx)
into the new file.
...
export function MainScreen() {
  return (
    <View>
      <Text>Nexis Native Chain Expo App</Text>
    </View>
  );
}
```
...
`react-native-get-random-values`. These are necessary for the Nexis Native Chain
dependencies to run correctly.

#### 4. Build and run Nexis Native Chain boilerplate
...
Everything should compile and you should have a boilerplate Nexis Native Chain Expo app.
...
import { Connection, Transaction } from "@nexis-network/web3.js";
  ...
  "expo": {
    ...
    "plugins": [
      [
        "expo-image-picker",
        {
          "photosPermission": "Allows you to use images to create nexis NFTs"
        }
      ]
    ],
    // ....
...
import { useConnection } from "./ConnectionProvider";
import { Connection, clusterApiUrl } from "@nexis-network/web3.js";
import { transact } from "@nexis-network-mobile/mobile-wallet-adapter-protocol";
import { Account, useAuthorization } from "./AuthProvider";
...
wallet and make sure you have some NZT. Next, tap on `Connect Wallet` and
...
[`main` branch in Github](https://github.com/Unboxed-Software/nexis-advance-mobile).
