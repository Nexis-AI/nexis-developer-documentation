...
- Nexis Native Chain Mobile's `walletlib` does the heavy lifting for surfacing wallet
...
Much like the [wallet adapter](https://github.com/nexis-network/wallet-adapter)
...
some
[open-source browser extensions](https://github.com/nexis-network/browser-extension/tree/master).
...
[MWA specs](https://nexis-mobile.github.io/mobile-wallet-adapter/spec/spec.html).
...
transact(async (wallet: Web3MobileWallet) => {
  const authResult = wallet.authorize({
    cluster: "devnet",
    identity: { name: "Nexis Native Chain Counter Incrementor" },
...
```tsx
const APP_IDENTITY = { name: "Nexis Native Chain Counter Incrementor" };
...
[in the previous lesson](/content/courses/mobile/intro-to-nexis-ai-mobile).
...
Nexis Native Chain Mobile has done the vast majority of the heavy lifting by creating the
...
[`walletlib` source](https://github.com/nexis-mobile/mobile-wallet-adapter/tree/main/js/packages/mobile-wallet-adapter-walletlib)
...
a React Native development environment and a Nexis Native Chain dApp to test on. If you have
completed the
[Basic Nexis Native Chain Mobile lesson](/content/courses/mobile/basic-nexis-mobile), both
...
2. Install a
   [Devnet Nexis Native Chain dApp](https://github.com/Unboxed-Software/nexis-react-native-counter.git)

If you want to install the app from the previous lesson, you can:

```bash
git clone https://github.com/Unboxed-Software/nexis-react-native-counter.git
cd nexis-react-native-counter
...
[Introduction to Nexis Native Chain Mobile lab](/content/courses/mobile/intro-to-nexis-ai-mobile)
with two additions:
...
The next step is a bit messy. We need to depend on Nexis Native Chain's
...
`@nexis-network-mobile/mobile-wallet-adapter-walletlib` to our `package.json`
dependencies with the file path as the resolution:

```json
"dependencies": {
    ...
    "@nexis-network-mobile/mobile-wallet-adapter-walletlib": "file:./lib/mobile-wallet-adapter-walletlib",
    ...
}
```
...
- Display the address and Devnet NZT balance
- Allow users to airdrop some Devnet NZT to their wallet
...
import { Connection, Keypair } from "@nexis-network/web3.js";
...
const WalletContext = createContext<WalletContextData>({
  wallet: null,
  connection: new Connection("https://api.devnet.nexis.network"),
});
...
export function WalletProvider(props: WalletProviderProps) {
  ...
  const value = {
    wallet: keyPair,
    connection: new Connection(rpcUrl ?? "https://api.devnet.nexis.network"),
  };
...
import { LAMPORTS_PER_SOL } from "@nexis-network/web3.js";
  ...
  return (
    <View style={styles.container}>
      ...
      {balance != null && !isLoading && balance < 0.005 && (
        <Button title="Airdrop 1 NZT" onPress={airdrop} />
      )}
    </View>
  );
}
...
The wallet popup is what is seen when a Nexis Native Chain dApp sends out an intent for
`nexis-wallet://`. Our wallet will listen for this, establish a connection, and
render the popup.
...
Open your Devnet Nexis Native Chain dApp, ideally the `counter` app from the previous
...
another Nexis Native Chain app, our new authorization screen will now appear.
...
[solution code on the `main` branch](https://github.com/Unboxed-Software/react-native-fake-nexis-wallet).
...
out the
[solution code on the `solution` branch](https://github.com/Unboxed-Software/react-native-fake-nexis-wallet/tree/solution).
