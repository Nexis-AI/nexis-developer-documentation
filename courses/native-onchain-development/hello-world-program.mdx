...
objectives:
  ...
  - Explain the entry point to a Nexis Native Chain program
  - Build and deploy a basic Nexis Native Chain program
...
- Native Nexis Native Chain programs have a single **entry point** to process instructions
...
<Callout type="info">
The following guide assumes you are familiar with Nexis Native Chain program basics - if not, check out [introduction to onchain programming](/developers/courses/onchain-development/intro-to-onchain).
</Callout>

This lesson will give you a basic introduction to writing and deploying a Nexis Native Chain
...
using a browser-based IDE called Nexis Native Chain Playground.
...
1. The base crate is `nexis_program`
2. `nexis_program` contains a module named `account_info`
3. `account_info` contains a struct named `AccountInfo`

The path to `AccountInfo` would be `nexis_program::account_info::AccountInfo`.
...
```rust
use nexis_program::account_info::AccountInfo
```
...
### Nexis Native Chain Programs

Recall that all data stored on the Nexis Native Chain network are contained in what are
referred to as accounts. Each account has its own unique address which is used
to identify and access the account data. Nexis Native Chain programs are just a particular
type of Nexis Native Chain account that store and execute instructions.

#### Nexis Native Chain Program Crate

To write Nexis Native Chain programs with Rust, we use the `nexis_program` library crate.
The `nexis_program` crate acts as a standard library for Nexis Native Chain programs. This
standard library contains the modules and macros that we'll use to develop our
Nexis Native Chain programs. If you want to dig deeper into the `nexis_program` crate, have
a look
[at the `nexis_program` crate documentation](https://docs.rs/nexis-program/latest/nexis_program/index.html).

For a basic program we will need to bring into scope the following items from
the `nexis_program` crate:
...
#### Nexis Native Chain Program Entry Point

Nexis Native Chain programs require a single entry point to process program instructions.
The entry point is declared using the `entrypoint!` macro.

The entry point to a Nexis Native Chain program requires a `process_instruction` function
...
Recall that Nexis Native Chain program accounts only store the logic to process
...
We're going to build a "Hello, World!" program using Nexis Native Chain Playground. Nexis Native Chain
Playground is a tool that allows you to write and deploy Nexis Native Chain programs from
the browser.

#### 1. Setup

Open the [Nexis Native Chain Playground](https://beta.solpg.io/). Next, go ahead and delete
...
![Gif Nexis Native Chain Playground Create Wallet](/public/assets/courses/unboxed/hello-world-create-wallet.gif)
</div>

#### 2. Nexis Native Chain Program Crate

First, let's bring into scope everything we’ll need from the `nexis_program`
crate.
...
Now let's build and deploy our program using Nexis Native Chain Playground.

<div class="p-2 not-prose relative bg-gray-50/50 rounded-2xl overflow-hidden dark:bg-gray-800/25">
<div class="absolute inset-0 bg-grid-neutral-200/20 [mask-image:linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] dark:bg-grid-white/5 dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]" style="background-position: 10px 10px;"></div>
![Gif Nexis Native Chain Playground Build and Deploy](/public/assets/courses/unboxed/hello-world-build-deploy.gif)
</div>

#### 5. Invoke Program

Finally, let's invoke our program from the client side. The focus of this lesson
is to build our Nexis Native Chain program, so we’ve gone ahead and provided
[the client code to invoke our “Hello, world!” program](https://github.com/Unboxed-Software/nexis-hello-world-client)
for you to download.

The code provided includes a `sayHello` helper function that builds and submits
our transaction. We then call `sayHello` in the main function and print a Nexis Native Chain
...
You can locate the program ID on Nexis Native Chain Playground referencing the image below.

<div class="p-2 not-prose relative bg-gray-50/50 rounded-2xl overflow-hidden dark:bg-gray-800/25">
<div class="absolute inset-0 bg-grid-neutral-200/20 [mask-image:linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] dark:bg-grid-white/5 dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]" style="background-position: 10px 10px;"></div>
![Gif Nexis Native Chain Playground Program ID](/public/assets/courses/unboxed/hello-world-program-id.gif)
</div>
...
2. Airdrop devnet NZT
3. Invoke the “Hello, world!” program
4. Output the transaction URL to view on Nexis Native Chain Explorer
...
<div class="p-2 not-prose relative bg-gray-50/50 rounded-2xl overflow-hidden dark:bg-gray-800/25">
<div class="absolute inset-0 bg-grid-neutral-200/20 [mask-image:linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] dark:bg-grid-white/5 dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]" style="background-position: 10px 10px;"></div>
![Screenshot Nexis Native Chain Explorer Program Log](/public/assets/courses/unboxed/hello-world-program-log.png)
</div>

Congratulations, you’ve just successfully built and deployed a Nexis Native Chain program!
...
3. Invoke your newly deployed program and use Nexis Native Chain Explorer to check that your
