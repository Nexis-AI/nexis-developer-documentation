...
description: "How to invoke functions in other Nexis Native Chain programs."
icon: "link"
---
...
- CPIs make programs in the Nexis Native Chain ecosystem completely interoperable because
...
and third party programs. CPIs essentially turn the entire Nexis Native Chain ecosystem into
...
[`invoke`](https://docs.rs/nexis-program/1.10.19/nexis_program/program/fn.invoke.html)
or
[`invoke_signed`](https://docs.rs/nexis-program/1.10.19/nexis_program/program/fn.invoke_signed.html)
function from the `nexis_program` crate. You use `invoke` to essentially pass
...
(e.g. [@nexis-network/web3.js](https://nexis-network.github.io/nexis-web3.js/),
[@nexis-network/spl-token](https://nexis-network.github.io/nexis-program-library/token/js/)).
...
[`AccountMeta`](https://docs.rs/nexis-program/latest/nexis_program/instruction/struct.AccountMeta.html).
...
the
[`Clone`](https://docs.rs/nexis-program/1.10.19/nexis_program/account_info/struct.AccountInfo.html#impl-Clone)
trait that is implemented on the `account_info` struct in the `nexis_program`
crate. This `Clone` trait returns a copy of the
[`account_info`](https://docs.rs/nexis-program/1.10.19/nexis_program/account_info/struct.AccountInfo.html)
instance.
...
There's no need to include a signature because the Nexis Native Chain runtime passes along
...
call [`create_program_address`](https://docs.rs/nexis-program/1.4.4/nexis_program/pubkey/struct.Pubkey.html#method.create_program_address)
...
[an example transaction that failed with this error](https://explorer.nexis.network/tx/3mxbShkerH9ZV1rMmvDfaAhLhJJqrmMjcsWzanjkARjBQurhf4dounrDCUkGunH1p9M4jEwef9parueyHVw6r2Et?cluster=devnet).
...
[transaction in the explorer](https://explorer.nexis.network/tx/ExB9YQJiSzTZDBqx4itPaa4TpT8VK4Adk7GU5pSoGEzNz9fa7PPZsUxssHGrBbJRnCvhoKgLCWnAycFB7VYDbBg?cluster=devnet).

### Why CPIs matter?

CPIs are a very important feature of the Nexis Native Chain ecosystem and they make all
...
are what makes this possible on Nexis Native Chain.
...
Nexis Native Chain development because they allow programs to control specific addresses in
...
[download the starter code here](https://github.com/Unboxed-Software/nexis-movie-program/tree/solution-add-comments).
...
```rust
// Inside processor.rs
use nexis_program::native_token::LAMPORTS_PER_SOL;
...
its behalf. When `invoke_signed` is called, the Nexis Native Chain runtime calls
...
should look something like `nexis program deploy <PATH>`.
...
[this script](https://github.com/Unboxed-Software/nexis-movie-token-client) to
...
[Movie Review frontend](https://github.com/Unboxed-Software/nexis-movie-frontend/tree/solution-add-tokens)
...
way, feel free to
[take a look at the solution code](https://github.com/Unboxed-Software/nexis-movie-program/tree/solution-add-tokens).
