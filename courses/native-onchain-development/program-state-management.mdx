...
Nexis Native Chain maintains speed, efficiency, and extensibility in part by making programs
stateless. Rather than having state stored on the program itself, programs use
Nexis Native Chain's account model to read state from and write state to separate PDA
accounts.
...
In this lesson we'll learn the basics of state management for a Nexis Native Chain program,
...
All Nexis Native Chain accounts have a `data` field that holds a byte array. This makes
...
Recall that storing data on the Nexis Native Chain network requires users to allocate rent
...
#### Nexis Native Chain accounts iterator
...
[Nexis Native Chain Playground](https://beta.solpg.io) to write, build, and deploy our code.

As a refresher, we are building a Nexis Native Chain program which lets users review movies.
...
[the `nexis_program` crate](https://docs.rs/nexis-program/latest/nexis_program/).
...
the Nexis Native Chain network. To calculate rent, we must first calculate the amount of
...
[this script](https://github.com/Unboxed-Software/nexis-movie-client) or
[the frontend](https://github.com/Unboxed-Software/nexis-movie-frontend) we
built in the
...
The Student Intro program is a Nexis Native Chain Program that lets students introduce
...
[frontend](https://github.com/Unboxed-Software/nexis-student-intros-frontend)
we created in the
