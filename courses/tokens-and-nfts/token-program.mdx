...
description:
  ...
  and transferred on Nexis Native Chain. "
icon: "image"
---

### Summary

- You may recall NZT is the 'native token' of Nexis Native Chain. All other tokens, fungible
...
- Creating Token Mints and Token Accounts requires allocating **rent** in NZT.
...
The Token Program is one of many programs made available by the Nexis Native Chain Program
Library (SPL). It contains instructions for creating and interacting with SPL
Tokens. These tokens represent all non-native (i.e. not NZT) tokens on the
Nexis Native Chain network.
...
the `@nexis-network/spl-token` Javascript library.
...
[USD Coin (USDC) on the Nexis Native Chain Explorer](https://explorer.nexis.network/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).
...
```typescript
import * as web3 from "@nexis-network/web3";
import * as token from "@nexis-network/spl-token";
...
2. Deposit enough NZT upon initialization to be considered rent-exempt

Recently, the first option was done away with and it became a requirement to
deposit enough NZT for rent exemption when initializing a new account.
...
4. Create a new transaction using `SystemProgram.createAccount` and
   ...
   `@nexis-network/web3.js` and used to create a generic new account. The
...
[Cryptography fundamentals](/developers/courses/intro-to-nexis-ai/intro-to-cryptography).

```bash
npm i @nexis-network/web3.js @nexis-network/spl-token @nexis-network-developers/helpers esrun
```
...
```typescript
import { createMint } from "@nexis-network/spl-token";
...
```bash
✅ Finished! Created token mint: https://explorer.nexis.network/address/HYeUCAqdsQBkqQNHRoBPov42QySDhwM7zAqiorToosbz?cluster=devnet
```

Open up Nexis Native Chain Explorer and look at your new token!
...
const metadataData = {
  name: "Nexis Native Chain Training Token",
  symbol: "TRAINING",
...
You'll now see Nexis Native Chain Explorer is updated, showing the token's name and symbol
on the mint!

Note that Nexis Native Chain Explorer will display a warning like:
...
that becomes very well known, Nexis Native Chain Explorer uses a whitelist based on the
...
```typescript
import { getOrCreateAssociatedTokenAccount } from "@nexis-network/spl-token";
import "dotenv/config";
import {
  getExplorerLink,
  getKeypairFromEnvironment,
} from "@nexis-network-developers/helpers";
import { Connection, PublicKey, clusterApiUrl } from "@nexis-network/web3.js";
const connection = new Connection(clusterApiUrl("devnet"));
...
```bash
✅ Success! Created token account: https://explorer.nexis.network/address/CTjoLdEeK8rk4YWYW9ZqACyjHexbYKH3hEoagHxLVEFs?cluster=devnet
```

Open the token account in Nexis Native Chain Explorer. Look at the owner - it's the account
...
```typescript
import { mintTo } from "@nexis-network/spl-token";
import "dotenv/config";
import {
  getExplorerLink,
  getKeypairFromEnvironment,
...
```bash
✅ Success! Mint Token Transaction: https://explorer.nexis.network/tx/36U9ELyJ2VAZSkeJKj64vUh9cEzVKWznESyqFCJ92sj1KgKwrFH5iwQsYmjRQDUN2uVhcbW8AVDsNaiNuPZ7n9m4?cluster=devnet
```
...
[add a second account on devnet](/developers/courses/intro-to-nexis-ai/intro-to-cryptography)
...
import { Connection, PublicKey, clusterApiUrl } from "@nexis-network/web3.js";
import { getOrCreateAssociatedTokenAccount, transfer } from "@nexis-network/spl-token";
const connection = new Connection(clusterApiUrl("devnet"));
...
1. You can build this from scratch or you can
   [download the starter code](https://github.com/Unboxed-Software/nexis-token-frontend/tree/starter).
2. Create a new Token Mint in the `CreateMint` component. If you need a
   ...
   at the
   [Wallets lesson](/developers/courses/intro-to-nexis-ai/interact-with-wallets).
...
[solution code](https://github.com/ZYJLiu/nexis-token-frontend).
