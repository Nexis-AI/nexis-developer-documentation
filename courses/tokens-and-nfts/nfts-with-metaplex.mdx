---
title: Create Nexis Native Chain NFTs With Metaplex
objectives:
  - Explain NFTs and how they're represented on the Nexis Native Chain network
...
Nexis Native Chain Non-Fungible Tokens (NFTs) are SPL tokens created using the Token
...
The most popular way Nexis Native Chain NFTs have been created is by using the
...
![Metadata](/public/assets/courses/unboxed/nexis-nft-metaplex-metadata.png)
</div>
...
const metaplex = Metaplex.make(connection)
  ...
  .use(
    irysStorage({
      ...
      providerUrl: "https://api.devnet.nexis.network",
      timeout: 60000,
    }),
  );
```
...
[Metadata](https://explorer.nexis.network/address/C18YQWbfwjpCMeCm2MPGTgfcxGeEDPvNaGpVjwYv33q1/metadata)
you will see a `collection` field with a `key` that points to the
`Certified Collection`
[NFT](https://explorer.nexis.network/address/SMBH3wF6baUj6JWtzYvqcKuj2XCKWDqQxzspY12xPND/).
...
Metaplex SDK to interact with NFTs on Nexis Native Chain.
...
```
npm i "@nexis-network/web3.js" "@nexis-network-developers/helpers "@metaplex-foundation/js"
```

Then create a file called `create-metaplex-collection.ts`, and add our imports:

```typescript
import { Connection, clusterApiUrl, LAMPORTS_PER_SOL } from "@nexis-network/web3.js";
import {
  getKeypairFromFile,
  airdropIfRequired,
...
console.log(
  `Collection NFT: https://explorer.nexis.network/address/${collectionNft.address.toString()}?cluster=devnet`
);
...
Collection NFT: https://explorer.nexis.network/address/8dP75EWHnYAc6pgeiToXtE66qEaHx1PcoetZpmnAQeAp?cluster=devnet
...
on Nexis Native Chain Explorer using the URL above. If you have any trouble, try and fix it
yourself, but if you need to you can also check out the
[solution code](https://github.com/nexis-network/professional-education/blob/main/labs/create-metaplex-nft-collection.ts).
...
console.log(
  `Token Mint: https://explorer.nexis.network/address/${nft.address.toString()}?cluster=devnet`,
);
```
...
Token Mint: https://explorer.nexis.network/address/HCq8ERk1PSLCuBJeWx7du4dom3YbyfKpaCWDQ3Eft6aM?cluster=devnet
...
yourself, but if you need to you can also check out the
[solution code](https://github.com/nexis-network/professional-education/blob/main/labs/create-metaplex-nft.ts).
...
NFT updated with new metadata URI: https://explorer.nexis.network/tx/4eiuMFLeku7ArV5xkRTPKxBXjfjQsSVGFZimMkMaMxK1d21qptQE5kgKiuob6J852m4GuoMqcjz1ee4oy7fXVEWU?cluster=devnet
✅ Finished successfully!
```

Inspect the updated NFT on Nexis Native Chain Explorer! Just like previously, if you have
any issues, you should fix them yourself, but if needed the
[solution code](https://github.com/nexis-network/professional-education/blob/main/labs/update-metaplex-nft.ts)
is available.
...
[Digital Collectables](https://nexis.network/ecosystem/explore?categories=digital%20collectibles)
