...
objectives:
  - Explain wallets
  - Install a Nexis Native wallet app and set your wallet app to
    [Devnet](https://api.devnet.nexis.network/)
...
- **Software wallets** use your computer for secure storage. On desktops,
  ...
  browsers.
- Nexis Native’s **Wallet Adapter** allows you to build websites that can request a
...
## Nexis Native’s Wallet Adapter

If you build web apps, and need users to be able to connect to their wallets and
sign transactions through your apps, you'll want Nexis Native’s Wallet Adapter. Wallet
Adapter is a suite of modular packages:

- The core functionality is found in `@nexisnetwork/wallet-adapter-base`.
- React support is added by `@nexisnetwork/wallet-adapter-react`.
...
appropriate packages. You’ll need `@nexisnetwork/wallet-adapter-base`,
`@nexisnetwork/wallet-adapter-react`. If you plan to use the provided React
...
```
npm install @nexisnetwork/wallet-adapter-base \
...
[create-nexis-dapp](https://github.com/nexis-network/create-nexis-dapp) to
create a brand new React or NextJS app that supports Nexis Native wallets. 
</Callout>

### Connect To Wallets

`@nexisnetwork/wallet-adapter-react` allows us to persist and access wallet connection
...
import * as web3 from "@nexisnetwork/web3.js";
...
provided by `@nexisnetwork/wallet-adapter-react-ui`. The simplest way to provide a
...
import {
  WalletModalProvider,
  WalletMultiButton,
} from "@nexisnetwork/wallet-adapter-react-ui";
import * as web3 from "@nexisnetwork/web3.js";
...
```tsx
import { useConnection, useWallet } from "@nexisnetwork/wallet-adapter-react";
import { LAMPORTS_PER_SOL } from "@nexisnetwork/web3.js";
...
export const BalanceDisplay: FC = () => {
  ...
  return (
    <div>
      <p>{publicKey ? `Balance: ${balance / LAMPORTS_PER_SOL} NZT` : ""}</p>
    </div>
  );
};
```
...
### Download a Nexis Native wallet

You'll need a Nexis Native wallet app. There's a wide variety of
[Nexis Native wallets](https://nexis.network/docs/intro/wallets) available. We're going
...
- In Phantom, click **Settings** -> **Developer Settings** -> **Testnet mode**.
  'Testnet mode' sets Nexis Native to Devnet by default.
...
Download the
[starter code for this project](https://github.com/Unboxed-Software/nexis-ping-frontend/tree/starter).
...
from `@nexisnetwork/wallet-adapter-react` and `@nexisnetwork/wallet-adapter-react-ui`. Then
...
import { WalletModalProvider } from "@nexisnetwork/wallet-adapter-react-ui";
...
`@nexisnetwork/web3.js` library that we’ve used before so you’ll need to import it.
For the array of wallets you’ll also need to import the
`@nexisnetwork/wallet-adapter-wallets` library.
...
`require('@nexisnetwork/wallet-adapter-react-ui/styles.css');` below your imports to
...
import { WalletModalProvider } from "@nexisnetwork/wallet-adapter-react-ui";
import * as web3 from "@nexisnetwork/web3.js";
import * as walletAdapterWallets from "@nexisnetwork/wallet-adapter-wallets";
...
import { WalletMultiButton } from "@nexisnetwork/wallet-adapter-react-ui";

export const AppBar: FC = () => {
  return (
    <div className={styles.AppHeader}>
      <Image src="/nexisLogo.png" height={30} width={200} />
...
`useWallet` from `@nexisnetwork/wallet-adapter-react`. While we’re here, let’s also
import `@nexisnetwork/web3.js` since we’ll need it to create our transaction.
...
transaction on Nexis Native Explorer after a user confirms a transaction so they can
...
[full source code from this lab](https://github.com/Unboxed-Software/nexis-ping-frontend)
to understand all of this in context.

## Challenge

Now it’s your turn to build something independently. Create an application that
lets a user connect their wallet and send NZT to another account.

<div class="p-2 not-prose relative bg-gray-50/50 rounded-2xl overflow-hidden dark:bg-gray-800/25">
<div class="absolute inset-0 bg-grid-neutral-200/20 [mask-image:linear-gradient(0deg,#fff,rgba(255,255,255,0.6))] dark:bg-grid-white/5 dark:[mask-image:linear-gradient(0deg,rgba(255,255,255,0.1),rgba(255,255,255,0.5))]" style="background-position: 10px 10px;"></div>
![Send NZT App](/public/assets/courses/unboxed/nexis-send-sol-app.png)
</div>

1. You can build this from scratch or you can
   [download the starter code](https://github.com/Unboxed-Software/nexis-send-sol-frontend/tree/starter).
...
4. Get creative with the user experience. Add a link to let the user view the
   transaction on Nexis Native Explorer or something else that seems cool to you!

If you get really stumped, feel free to
[check out the solution code](https://github.com/Unboxed-Software/nexis-send-sol-frontend/tree/main).
