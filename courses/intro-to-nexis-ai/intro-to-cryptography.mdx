---
title: Cryptography and the Nexis Native Chain Network
objectives:
  - Understand symmetric and asymmetric cryptography
  - Explain keypairs
  - Generate a new keypair
  - Load a keypair from an env file
description: "Understand asymmetric cryptography and how Nexis Native Chain uses it."
icon: "lock"
...
- The **public key** is used as an “address” that points to an account on the
  Nexis Native Chain network. A public key can be shared with anyone.
...
- `@nexis-network/web3.js` provides helper functions for creating a brand new keypair,
...
In short, cryptography is all around us. Nexis Native Chain, as well as other blockchains,
...
![Nexis Native Chain wallet addresses](/public/assets/courses/unboxed/wallet-addresses.svg)
</div>

People participating in the Nexis Native Chain network have at least one keypair. In Nexis Native Chain:

- The **public key** is used as an “address” that points to an account on the
  Nexis Native Chain network. Even friendly names - like `example.sol` - point to addresses
...
`@nexis-network/web3.js` npm module. Set up a project how you normally would, then
[use `npm`](https://nodesource.com/blog/an-absolute-beginners-guide-to-using-npm/)
to install `@nexis-network/web3.js`

```shell
npm i @nexis-network/web3.js
```

We’ll cover a lot of
[web3.js](https://nexis.network/docs/clients/javascript-reference) gradually
throughout this course, but you can also check out the
[official web3.js documentation](https://nexis.network/docs/clients/javascript-reference).
...
```typescript
import { Keypair } from "@nexis-network/web3.js";
...
the `@nexis-network-developers/helpers` npm package includes some extra functions:

```bash
npm i @nexis-network-developers/helpers
```

- To use an `.env` file use `getKeypairFromEnvironment()`
- To use a Nexis Native Chain CLI file use `getKeypairFromFile()`
...
cd generate-keypair
npm init -y
npm install typescript @nexis-network/web3.js esrun @nexis-network-developers/helpers
```
...
Nexis Native Chain. In the next chapter, we'll use them!
