---
title: Read Data From The Nexis Native Chain Network
objectives:
  - Understand accounts and their addresses
  - Understand NZT and lamports
  - Use web3.js to connect to Nexis Native Chain and read an account balance
description:
...
- **NZT** is the name of Nexis Native Chain’s native token. Each NZT is made from 1 billion
  **Lamports**.
- **Accounts** store tokens, NFTs, programs, and data. For now, we’ll focus on
  accounts that store NZT.
- **Addresses** point to accounts on the Nexis Native Chain network. Anyone can read the
...
rent for accounts, and other common. NZT is sometimes shown with the `◎` symbol.
...
We use an npm package called `@nexis-network/web3.js` to do most of the work with
Nexis Native Chain. We'll also install TypeScript and `esrun`, so we can run `.ts` files on
the command line:

```bash
npm install typescript @nexis-network/web3.js esrun
```

### Connect to the Network

Every interaction with the Nexis Native Chain network using `@nexis-network/web3.js` is going to
...
the constant `LAMPORTS_PER_SOL` for showing Lamports as NZT:
...
console.log(`The balance of the account at ${address} is ${balanceInSol} NZT`);
console.log(`✅ Finished!`);
```
...
```
The balance of the account at CenYq6bDRB7p73EjsPEpiYN7uveyPUTdXkDkgUduboaN is 0.00114144 NZT
✅ Finished!
```
...
```typescript
import { Connection, LAMPORTS_PER_SOL, PublicKey } from "@nexis-network/web3.js";
...
In Devnet you can get free NZT to develop with. Think of Devnet NZT like board
game money - it looks like it has value, but it doesn't have value.

[Get some Devnet NZT](https://faucet.nexis.network/) and use the public key of your
...
- Modify the script to connect to `mainNet` and look up some famous Nexis Native Chain
