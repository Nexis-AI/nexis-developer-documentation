...
This can be particularly difficult in Nexis Native Chain program development. For example,
...
concept of environment variables in a Nexis Native Chain program. If there were, you could
...
of [this repository](https://github.com/Unboxed-Software/nexis-admin-instructions/tree/starter).
...
Generate a new keypair by running `nexis-keygen grind`. Run the following
command to generate a keypair with a public key that begins with "env".

```
nexis-keygen grind --starts-with env:1
```
...
2. Include an `ADMIN` constant, which will be used as a constraint when
   initializing the program config account. Run the `nexis address` command to
...
the `solution` branch
of [the same repository](https://github.com/Unboxed-Software/nexis-admin-instructions/tree/solution).
...
test validator using `nexis-test-validator`. This test validator uses a
...
const deploy = () => {
  const deployCmd = `nexis program deploy --url localhost -v --program-id $(pwd)/target/deploy/config-keypair.json $(pwd)/target/deploy/config.so`
  execSync(deployCmd)
}
...
`challenge` branch of
[the same repository](https://github.com/Unboxed-Software/nexis-admin-instructions/tree/challenge)
