...
By design, Nexis Native Chain transactions are limited to 1232 bytes. Transactions exceeding
...
To help get around the transaction size limitation, Nexis Native Chain released a new
transaction format that allows support for multiple versions of transaction
formats. At the time of writing, Nexis Native Chain supports two transaction versions:

1. `legacy` - the original transaction format
2. `0` - the newest transaction format that includes support for Address Lookup
   Tables

Versioned transactions don't require any modifications to existing Nexis Native Chain
...
One of the items taking up the most space in Nexis Native Chain transactions is the
...
Nexis Native Chain released versioned transactions to support multiple transaction formats.
Alongside the release of versioned transactions, Nexis Native Chain released version 0 of
...
To simplify the process of working with LUTs, the `@nexis-network/web3.js` library
...
[repository](https://github.com/Unboxed-Software/nexis-versioned-transactions/tree/starter).
...
async function main() {
  ...
  // Log the transaction URL on the Nexis Native Chain Explorer
  console.log(`https://explorer.nexis.network/tx/${txid}?cluster=devnet`);
}
```

To execute the code, run `npm start`. This will create a new keypair, write it
to the `.env` file, airdrop devnet NZT to the keypair, and send the transaction
...
- Retrieves the latest blockhash and last valid block height from the Nexis Native Chain
  network
- Creates a new transaction message using the provided instructions
- Signs the transaction using the user's keypair
- Sends the transaction to the Nexis Native Chain network
- Confirms the transaction
- Logs the transaction URL on the Nexis Native Chain Explorer
...
https://explorer.nexis.network/tx/4JvCo2azy2u8XK2pU8AnJiHAucKTrZ6QX7EEHVuNSED8B5A8t9GqY5CP9xB8fZpTNuR7tbUcnj2MiL41xRJnLGzV?cluster=devnet
Waiting for 1 new blocks
https://explorer.nexis.network/tx/rgpmxGU4QaAXw9eyqfMUqv8Lp6LHTuTyjQqDXpeFcu1ijQMmCH2V3Sb54x2wWAbnWXnMpJNGg4eLvuy3r8izGHt?cluster=devnet
...
[find it on the solution branch](https://github.com/Unboxed-Software/nexis-versioned-transactions/tree/solution).
