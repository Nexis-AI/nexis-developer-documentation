...
Nexis Native Chain’s transaction costs are so cheap that most users never think about how
expensive minting NFTs can be at scale. The cost to set up and mint 1 million
traditional NFTs is approximately 24,000 NZT. By comparison, cNFTs can be
structured to where the same setup and mint costs 10 NZT or less. That means
...
and composable throughout the Nexis Native Chain ecosystem. It provides instructions for
...
The Nexis Native Chain ledger is a list of entries containing signed transactions. In
...
Fortunately, you can use tools created by Nexis Native Chain Foundation, the Nexis Native Chain
developer community, and Metaplex to simplify the process. Specifically, we’ll
be using the `@nexis-network/spl-account-compression` SDK, the Metaplex Bubblegum
program, and the Bubblegum program’s corresponding TS SDK
`@metaplex-foundation/mpl-bugglegum`.

<aside>
💡 At the time of writing, the Metaplex team is transitioning to a new bubblegum client SDK that supports umi, their modular framework for building and using JS clients for Nexis Native Chain programs. We will not be using the umi version of the SDK in this lesson. Rather, we’ll be hardcoding our dependency to version 0.7 (`@metaplex-foundation/mpl-bubblegum@0.7`). This version provides simple helper functions for building Bubblegum instructions.
...
from the `@nexis-network/spl-account-compression` TS SDK to create the instruction for
creating the empty account.

```typescript
import { createAllocTreeIx } from "@nexis-network/spl-account-compression"
...
import {
  ...
  SPL_NOOP_PROGRAM_ID,
} from "@nexis-network/spl-account-compression"
import {
...
[Digital Asset Standard Read API](https://nexis.network/developers/guides/javascript/compressed-nfts#reading-compressed-nfts-metadata)
...
3. Fetch the Merkle tree account from the Nexis Native Chain blockchain
...
[cNFT lab repository](https://github.com/Unboxed-Software/nexis-cnft-demo).

`git clone https://github.com/Unboxed-Software/nexis-cnft-demo.git`
...
- `airdropSolIfNeeded` will airdrop some Devnet NZT to a specified address if
  that address’s balance is below 1 NZT.
...
- `maxDepthSizePair` - a `ValidDepthSizePair`. This type comes from
  `@nexis-network/spl-account-compression`. It’s a simple object with properties
...
async function createAndInitializeTree(
  ...
  try {
    ...
    console.log(`https://explorer.nexis.network/tx/${txSignature}?cluster=devnet`);
...
Keep in mind that Devnet NZT is throttled so if you test too many times you
might run out of Devnet NZT before we get to minting. To test, in your terminal
run the following:
...
async function mintCompressedNftToCollection(
  ...
  for (let i = 0; i < amount; i++) {
    ...
    try {
      ...
      console.log(
        `https://explorer.nexis.network/tx/${txSignature}?cluster=devnet`,
      );
    } catch (err) {
...
[Read API](https://nexis.network/developers/guides/javascript/compressed-nfts) to
...
larger values and as long as you have enough Devnet NZT, this script will let
...
[lab repo](https://github.com/Unboxed-Software/nexis-cnft-demo/tree/solution).
