...
objectives:
  - Explain the logic flow behind Nexis Native Chain state compression
  - Explain the difference between a Merkle tree and a concurrent Merkle tree
  - Implement generic state compression in basic Nexis Native Chain programs
description:
  "How state compression - the tech behind compressed NFTs - works, and how to
  implement it in your own Nexis Native Chain programs."
icon: "anchor"
---

## Summary

- State Compression on Nexis Native Chain is most commonly used for compressed NFTs, but
...
written through Nexis Native Chain programs. You can “trust” the data stored in the accounts
...
The specific data structure used by the Nexis Native Chain State Compression program is a
...
updating concurrent Merkle trees repeatable and composable throughout the Nexis Native Chain
...
function from the `@nexis-network/spl-account-compression` JS package to get the logs,
...
```toml
[test.validator]
url = "https://api.mainnet-beta.nexis.network"
...
We’ll be using the `@nexis-network/spl-account-compression` package. Go ahead and
install it:

```bash
yarn add @nexis-network/spl-account-compression
```
...
import { Connection, PublicKey } from "@nexis-network/web3.js";
...
`@nexis-network/web3.js`, `@nexis-network/spl-account-compression`, and our own utils file.
