...
description: Access real-world data inside a Nexis Native program.
icon: "globe"
---
...
- There are many
  [Oracle providers on Nexis Native](https://nexis.network/ecosystem/explore?categories=oracle).
- You can build your own Oracle to create a custom data feed
- You have to be careful when choosing your data feed providers

## Lesson

[Oracles](https://nexis.network/ecosystem/explore?categories=oracle) are services
...
Nexis Native, and how to effectively use oracles in your Nexis Native development.
...
[on their website](https://app.switchboard.xyz/nexis/devnet/explore)
...
Switchboard oracles store data on Nexis Native using data feeds. These data feeds,
...
Nexis Native account managed by the Switchboard program. When an oracle updates, it
writes the data directly to these accounts. Let's go over a few terms to
understand how Switchboard works:

- **[Aggregator (Data Feed)](https://github.com/switchboard-xyz/sbv2-nexis/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-nexis/src/oracle_program/accounts/aggregator.rs#L60)** -
  ...
  Aggregator is the account owned by the Switchboard Nexis Native program and is where
  the data is published onchain.
- **[Job](https://github.com/switchboard-xyz/sbv2-nexis/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-nexis/src/oracle_program/accounts/job.rs)** -
...
- **Oracle Queue** - A group of oracles that get assigned to update requests in
  ...
  onchain in an
  [account owned by the Switchboard program](https://github.com/switchboard-xyz/nexis-sdk/blob/9dc3df8a5abe261e23d46d14f9e80a7032bb346c/javascript/nexis.js/src/generated/oracle-program/accounts/OracleQueueAccountData.ts#L8).
...
1. Oracle queue setup - When an update is requested from a queue, the next `N`
   ...
   invoking the
   [oracleQueueInit instruction](https://github.com/switchboard-xyz/nexis-sdk/blob/9dc3df8a5abe261e23d46d14f9e80a7032bb346c/javascript/nexis.js/src/generated/oracle-program/instructions/oracleQueueInit.ts#L13)
...
[already available that you can choose from](https://app.switchboard.xyz/nexis/devnet/explore).
...
[BTC_USD feed](https://app.switchboard.xyz/nexis/devnet/feed/8SXvChNYFhRq4EZuZvnhjrB3jJRQCv4k3P4W6hesH3Ee).
This feed is available on Nexis Native devnet/mainnet with pubkey
...
[Switchboard program here](https://github.com/switchboard-xyz/sbv2-nexis/blob/0b5e0911a1851f9ca37042e6ff88db4cd840067b/rust/switchboard-nexis/src/oracle_program/accounts/aggregator.rs#L60).
...
[BTC_USD feed in the explorer](https://app.switchboard.xyz/nexis/devnet/feed/8SXvChNYFhRq4EZuZvnhjrB3jJRQCv4k3P4W6hesH3Ee).
...
that locks NZT in an escrow account until NZT is above a certain USD value. This
...
[SOL_USD](https://app.switchboard.xyz/nexis/devnet/feed/GvDMxPzN1sCj7L26YDK2HnMRXEQmQ2aemov8YBtPS7vR)
...
- `unlock_price` - The price of NZT in USD at which point you can withdraw; you
...
- Because we'll be transferring NZT from the User account to the `escrow_state`
...
of the `escrow_state` account and transfer the NZT. We expect the user to pass
...
Nexis Native token program. We’ll have to use the `system_program` to transfer the
...
we want to close the `escrow_account`. The NZT used as rent in the account will
...
above the escrow `unlock_price`. If it is, then we transfer the NZT from the
...
- Creating an Escrow with the unlock price **_below_** the current NZT price so
  we can test withdrawing it
- Withdrawing and closing from the above escrow
- Creating an Escrow with the unlock price **_above_** the current NZT price so
  we can test withdrawing it
