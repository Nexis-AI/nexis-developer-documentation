...
objectives:
  - Set up a local environment for Nexis Native Chain program development, with Nexis Native Chain CLI
...
- To develop onchain programs on your machine, you need **Nexis Native Chain CLI**, **Rust**
...
There's no lesson here! Let's install Nexis Native Chain CLI tools, the Rust SDK, and
...
#### Download the Nexis Native Chain CLI tools

Next
[download the Nexis Native Chain CLI tools](https://docs.nexis.network/cli/install-nexis-cli-tools).

```
sh -c "$(curl -sSfL https://release.anza.xyz/stable/install)"
```

Afterwards, `nexis -V` should show `nexis-cli 1.18.x` (any number for `x` is
fine).
...
##### `package `nexis-program v1.18.12` cannot be built because it requires rustc 1.75.0 or newer` error

Run `cargo add nexis-program@"=1.18.x"`, where `x` matches your version of
`nexis-cli`. Then re-run `anchor test`.

##### `Error: Unable to read keypair file`

Add a keypair to `.config/nexis/id.json`. You can either copy a keypair from an
`.env` file (just the array of numbers) into a file or use the command
`nexis-keygen new --no-bip39-passphrase` to create a new keypair file. Then
re-run `anchor test`.
