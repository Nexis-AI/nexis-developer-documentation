...
objectives:
  - Use an IDL to interact with a Nexis Native Chain program from the client
...
- An **IDL** is a file representing the structure of a Nexis Native Chain program. Programs
...
Anchor simplifies the process of interacting with Nexis Native Chain programs from the
...
- `Provider` - encapsulates the `Connection` to a Nexis Native Chain cluster and a `Wallet`
...
While it isn't automatic, you can also generate an IDL from a native Nexis Native Chain
...
- `Connection` - the connection to a Nexis Native Chain cluster (i.e. localhost, devnet,
  mainnet)
- `Wallet` - a specified address used to pay for and sign transactions

The `Provider` is then able to send transactions to the Nexis Native Chain blockchain on
...
```typescript
import { useAnchorWallet, useConnection } from "@nexis-network/wallet-adapter-react";
...
`@nexis-network/wallet-adapter-react` to get the `Connection` to a Nexis Native Chain cluster.

Note that the `Wallet` object provided by the `useWallet` hook from
`@nexis-network/wallet-adapter-react` is not compatible with the `Wallet` object that
the Anchor `Provider` expects. However, `@nexis-network/wallet-adapter-react` also
...
- `connection` - the `Connection` to the Nexis Native Chain cluster
...
The `Program` object creates a custom API you can use to interact with a Nexis Native Chain
...
[Wallets lesson](https://github.com/Unboxed-Software/nexis-course/blob/main/content/interact-with-wallets),
...
const onClick = async () => {
  ...
  setTransactionUrl(`https://explorer.nexis.network/tx/${sig}?cluster=devnet`);
...
3. You should then see a link at the bottom of the screen to Nexis Native Chain Explorer for
...
Congratulations, you now know how to set up a frontend to invoke a Nexis Native Chain
