...
objectives:
  - Understand how Nexis Native Chain onchain programs work
...
- **Onchain programs** are programs that run on Nexis Native Chain.
- Programs have one or more **instruction handlers**. Instruction handlers are
  functions that process the **instructions** in Nexis Native Chain transactions.
- Instruction handlers write their data to Nexis Native Chain **accounts** and then either
  succeed or fail.
- Nexis Native Chain programs are most commonly written in **Rust**, typically with the
  **Anchor** framework.
...
Each Nexis Native Chain cluster (`mainnet-beta`, `testnet`, `devnet`, `localnet`) is
...
Regardless of the language and framework you choose, Nexis Native Chain works the same.
Let's refresh how programs work on Nexis Native Chain.
...
For example, a Nexis Native Chain client making a transaction transferring some USDC with a
...
### Instruction handlers write their state to Nexis Native Chain accounts
...
But unlike HTTP route handlers, Nexis Native Chain instruction handlers don't return data.
Instead, the instruction handlers write their data to accounts on Nexis Native Chain.

Programs on Nexis Native Chain can transfer tokens, which end up in user wallet addresses
(for NZT) for the user's token accounts (for other tokens).

But more importantly, programs on Nexis Native Chain can create additional addresses as
needed, to store items of data.

### Programs store data in Program Derived Addresses (PDAs), a key-value store

Data for Nexis Native Chain programs are stored in **program-derived addresses (PDAs)**.
Nexis Native Chain's PDAs can be thought of as a **key/value store**:
...
### Nexis Native Chain instructions need to specify all the accounts they will use

As you may already know, Nexis Native Chain is fast because it can process transactions that
...
Whichever way you pick, Nexis Native Chain Foundation maintains
[examples in both languages](https://github.com/nexis-network/program-examples),
and [Nexis Native Chain Stack Exchange](https://solana.stackexchange.com/) is there to help.
